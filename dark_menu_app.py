import os, csv, io
from flask import Flask, request, redirect, render_template_string, session, send_from_directory

app = Flask(__name__)
app.secret_key = "absolute_final_fix"

# --- CONFIG ---
DATA_FILE = "menu_data.csv"
UPLOAD_DIR = "uploads"
ADMIN_PASS = "1234"
# Using simple internal keys to avoid any mapping errors
COLS = ["C_EN", "C_AR", "C_KU", "N_EN", "N_AR", "N_KU", "D_EN", "D_AR", "D_KU", "Price", "Img"]

if not os.path.exists(UPLOAD_DIR): os.makedirs(UPLOAD_DIR)

def load_db():
    if not os.path.exists(DATA_FILE): return []
    try:
        with open(DATA_FILE, "r", encoding="utf-8-sig") as f:
            reader = csv.DictReader(f)
            return [row for row in reader]
    except: return []

def save_db(data):
    with open(DATA_FILE, "w", encoding="utf-8-sig", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=COLS)
        writer.writeheader()
        writer.writerows(data)

# --- THE "NO-MISTAKE" SYNC ---
def sync_by_index(file_stream, lang):
    stream = io.StringIO(file_stream.read().decode("utf-8-sig"))
    # We convert to a list of lists to ignore header names and use column index [0, 1, 2, 3]
    raw_rows = list(csv.reader(stream))
    data_rows = raw_rows[1:] # Skip the first row (header)
    
    db = load_db()
    # If DB is shorter than the file, grow the DB
    while len(db) < len(data_rows):
        db.append({c: "" for c in COLS})
        
    for i, row in enumerate(data_rows):
        if len(row) < 2: continue # Skip empty lines
        
        # Mapping by Column Position: 0=Cat, 1=Name, 2=Desc, 3=Price
        db[i][f"C_{lang}"] = row[0].strip()
        db[i][f"N_{lang}"] = row[1].strip()
        db[i][f"D_{lang}"] = row[2].strip() if len(row) > 2 else ""
        if len(row) > 3: 
            db[i]["Price"] = row[3].strip()
            
    save_db(db)

# --- HTML ---
BASE_HTML = r'''
<!DOCTYPE html>
<html lang="{{l}}" dir="{{ 'rtl' if l in ['ar','ku'] else 'ltr' }}">
<head>
    <meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
    <style>
        body { background:#0a0a0b; color:#fff; font-family:sans-serif; margin:0; }
        .header { text-align:center; padding:20px; border-bottom:1px solid #222; }
        .nav { display:flex; justify-content:center; gap:15px; margin:15px 0; }
        .nav a { color:#888; text-decoration:none; font-size:14px; }
        .nav a.active { color:gold; font-weight:bold; }
        .item { padding:15px; border-bottom:1px solid #1a1a1a; display:flex; gap:15px; max-width:600px; margin:auto; }
        .item img { width:80px; height:80px; object-fit:cover; border-radius:8px; }
        .price { color:gold; font-weight:bold; }
    </style>
</head>
<body>
    <div class="header">
        <div class="nav">
            <a href="?l=en" class="{{'active' if l=='en'}}">EN</a>
            <a href="?l=ar" class="{{'active' if l=='ar'}}">AR</a>
            <a href="?l=ku" class="{{'active' if l=='ku'}}">KU</a>
        </div>
    </div>
    {% for i in items %}
    <div class="item">
        {% if i.Img %}<img src="/uploads/{{i.Img}}">{% endif %}
        <div style="flex:1">
            <div style="display:flex; justify-content:space-between">
                {# FALLBACK LOGIC: If current lang name is empty, show EN name #}
                <b style="font-size:1.1rem;">{{ i['N_'~l|upper] or i['N_EN'] or "Unnamed Item" }}</b>
                <span class="price">{{ i.Price }}</span>
            </div>
            <p style="color:#888; margin:5px 0;">{{ i['D_'~l|upper] or i['D_EN'] }}</p>
        </div>
    </div>
    {% endfor %}
</body>
</html>
'''

ADMIN_HTML = r'''
<body style="background:#111; color:#fff; font-family:sans-serif; padding:20px;">
    <h2>Admin Manager</h2>
    <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:10px; margin-bottom:20px;">
        <form action="/sync/EN" method="post" enctype="multipart/form-data" style="background:#222; padding:10px;">
            EN CSV: <input type="file" name="f" required><button>Sync</button>
        </form>
        <form action="/sync/AR" method="post" enctype="multipart/form-data" style="background:#222; padding:10px;">
            AR CSV: <input type="file" name="f" required><button>Sync</button>
        </form>
        <form action="/sync/KU" method="post" enctype="multipart/form-data" style="background:#222; padding:10px;">
            KU CSV: <input type="file" name="f" required><button>Sync</button>
        </form>
    </div>
    <table style="width:100%; border-collapse:collapse;">
        <tr style="text-align:left; background:#333;"><th>Row</th><th>Name (EN)</th><th>Price</th><th>Action</th></tr>
        {% for i in items %}
        <tr style="border-bottom:1px solid #333;">
            <td>{{ loop.index }}</td>
            <td>{{ i.N_EN }}</td>
            <td>{{ i.Price }}</td>
            <td>
                <form action="/photo/{{loop.index0}}" method="post" enctype="multipart/form-data" style="display:inline;">
                    <input type="file" name="f" required><button>Pic</button>
                </form>
                | <a href="/del/{{loop.index0}}" style="color:red;">Del</a>
            </td>
        </tr>
        {% endfor %}
    </table>
</body>
'''

# --- ROUTES ---
@app.route("/")
def index():
    l = request.args.get("l", "en")
    return render_template_string(BASE_HTML, items=load_db(), l=l)

@app.route("/admin")
def admin():
    if not session.get("auth"): return redirect("/login")
    return render_template_string(ADMIN_HTML, items=load_db())

@app.route("/login", methods=["GET", "POST"])
def login():
    if request.method == "POST" and request.form.get("p") == ADMIN_PASS:
        session["auth"] = True; return redirect("/admin")
    return '<body><form method="post">Pass: <input type="password" name="p"><button>Go</button></form></body>'

@app.route("/sync/<lang>", methods=["POST"])
def do_sync(lang):
    f = request.files.get("f")
    if f: sync_by_index(f, lang.upper())
    return redirect("/admin")

@app.route("/photo/<int:idx>", methods=["POST"])
def do_photo(idx):
    f = request.files.get("f")
    if f:
        ext = f.filename.rsplit('.', 1)[-1].lower()
        name = f"img_{idx}.{ext}"
        f.save(os.path.join(UPLOAD_DIR, name))
        db = load_db(); db[idx]["Img"] = name; save_db(db)
    return redirect("/admin")

@app.route("/del/<int:idx>")
def do_del(idx):
    db = load_db(); db.pop(idx); save_db(db); return redirect("/admin")

@app.route("/uploads/<path:filename>")
def get_file(filename): return send_from_directory(UPLOAD_DIR, filename)

if __name__ == "__main__":
    if not os.path.exists(DATA_FILE): save_db([])
    app.run(debug=True)
